...
class ezgal(object):
    ...
    def get_mass_weighted_ages( self, zf, zs=None, units='gyrs' ):
		...
		for i in range( zs.size ):
			...
			outgoing_ages[i] = np.sum( (incoming_ages[i]-self.ages[m])*self.sfh[m] )/np.sum( self.sfh[m] )

		return utils.convert_time( outgoing_ages, incoming='yrs', outgoing=units )
    ...
    def _check_grid( self, filters=None, zfs=None, solar=False, masses=False ):
		...
		if self.nvs and self.nages:
			self._grid_filters( filters=filters, zfs=zfs, solar=solar, masses=masses )
		else:
    ...
    def _grid_filters( self, filters=None, zfs=None, force=False, solar=False, masses=False ):
		...
		if self.nages == 0 or self.nvs == 0: raise ValueError( 'Cannot grid filters because no seds were found in the model file!' )
		...
		for filter in filters:
			...
			if not self.filters.has_key( filter ):
				...
				if not self.filters.has_key( filter ): raise ValueError( 'The specified filter, %s, has not been loaded and cannot be found!' % filter )
			...
			for zf in zfs:
				...
				if self.filters[filter].has_zf( zf ) and not( force ):
					...
					if solar and not self.filters[filter].has_zf( zf, solar=True ):
						# if ezgal doesn't have solar mags then there is a problem...
						if not self.has_solar: raise ValueError( 'Cannot grid solar magnitudes - solar spectrum is not loaded!' )
						# okay, grid the solar mags
						self.filters[filter].grid_solar( zf, self.solar[:,0], self.solar[:,1] )

					# grid masses if requested and needed
					if masses and not self.filters[filter].has_zf( zf, masses=True ):
						# if ezgal doesn't have masses then there is a problem...
						if not self.has_masses: raise ValueError( 'Cannot grid masses: mass-age relationship is not loaded!' )
						# okay, grid the masses
						self.filters[filter].grid_masses( zf, self.ages, self.masses )
				...
				self.filters[filter].grid( zf, self.vs, zs, ages, seds, force=force )

				# grid solar mags if ezgal has them
				if self.has_solar: self.filters[filter].grid_solar( zf, self.solar[:,0], self.solar[:,1] )

				# and grid masses if ezgal has them
				if self.has_masses: self.filters[filter].grid_masses( zf, self.ages, self.masses )
    ...
    def get_age( self, z1, z2, units='gyrs' ):
		...
		if type( z2 ) == type( [] ) or type( z2 ) == type( np.array( [] ) ):
			...
			for i in range( len( z2 ) ): ages[i] = utils.to_years( self.cosmo.Tl( z1, yr=True )-self.cosmo.Tl( z2[i], yr=True ), units=units, reverse=True )
			return ages
		else:
